...
> - Why raster data?
    ...
    * Earth Observation data (visible: land, ocean; temperature; radar; atmospheric composition) now come down, freely, in Copernicus now with [150 Tb/day](https://twitter.com/AschbacherJosef/status/1081277761430401024)
...
* list of `base::array`s with measurements
* list of vectors with dimension values
* mimics heterogeneous array records
...
* Raster data cubes, where x and y take a spatial dimension, (raster data)
* Vector data cubes,  where a set of feature geometries (points, lines, polygons) form the values of (at least) one dimension

and further

* follow `tbl_cube`, is a list of arrays and a set of dimensions
...
* supports out-of-core and  (`stars_proxy`)
...
* allows integrating non-aligned times, geometries, and coordinate reference systems (different sensors, or e.g. UTM zones for Sentinel2)
* largely inspired by Google Earth Engine

## Beyond sf and stars: 1

Exciting areas, so far barely touched upon in tidyverse context:
...
ggplot() + geom_sf(data = sf, aes(col = year)) + facet_wrap(~year_grp)
```
