...
<img src="asdar.jpg" width="130" />

- recently developed an interest in working with measurement `units`
...
- derived geometries: `intersection`\*, `difference`\*, `union`, `sym_difference`
- \* : also for n-ary sets
- $O(N^2)$ behaviour is avoided by using spatial indexes

## Intersection special cases

<img src="empty.png" width="720" />

- (typed) `EMPTY`: think of as `NA` (missing) geometry
- `GEOMETRYCOLLECTION`: mix of anything

- implemented as geometry list-column in `data.frame` or `tibble`
...
- `sfg` object (e.g. with class `c("XY", "POINT", "sfg")`) hold a single feature's geometry
- `sfc` objects are classed list-columns of `sfg` objects, with attributes `bbox`, `crs` and `precision` 
- `sf` objects subclass `data.frame` or `tbl_df`,  with attributes `sf_column` and `agr`


## Operations on attribute + geometry

- `st_join`, join tables based on a binary spatial predicate
- `summarise`, `aggregate`: aggregates attribute, unions geometry
...
- `sf` methods of tidyverse have:
-  `filter`, `arrange`, `distinct`, `group_by`, `ungroup`, `mutate`, `select` have
sticky geometry 
-  `summarise` unions geometry by group (or alltogether)
...
## Geometry predicates and operations, `st_join`

show mini figures for intersection / union / difference
